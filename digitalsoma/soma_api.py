...
_ANIMAL_TYPE_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "porcine_adult": {
        "taxa": "Sus scrofa domesticus",
        "ncbi_taxon_id": "9823",
        "body_mass_kg": 90.0,         # commercial finishing weight ~90 kg
        "core_temp_normal_C": 38.8,   # normal range 38.0–39.5 °C
        "hr_normal_bpm": 75.0,        # normal range 60–90 bpm
        "rr_normal_bpm": 20.0,        # normal range 15–25 bpm
        "cortisol_normal_nmol_L": 40.0,  # resting baseline
        "systems": ["cardiovascular", "metabolic", "thermoregulation",
                    "respiratory", "musculoskeletal", "neuroendocrine",
                    "immune"],
        # Clinical notes:
        #   Sus scrofa has limited sweat glands — heat dissipation is
        #   primarily via respiratory panting and wallowing behaviour.
        #   TCI rises rapidly in warm environments; hyperthermic risk
        #   above 28°C ambient in confined housing.
        #   Cardiac anatomy closely mirrors human — widely used as
        #   cardiovascular research model (NCBITaxon: 9823).
    },
    "equine_adult": {
        "taxa": "Equus caballus",
        "ncbi_taxon_id": "9796",
...
class DigitalSoma:
    ...
    def to_fhir_bundle(self, bundle_type: str = "collection") -> Dict[str, Any]:
        """
        Export current physiological state as a HL7 FHIR R4 Bundle.

        Produces:
            - Patient       : animal subject with NCBITaxon species extension
            - Device        : DigitalSoma twin identifier
            - Observation   : one per mapped canonical state property
                              (LOINC + SNOMED CT coded, UCUM units)
            - DiagnosticReport : VeDDRA adverse event findings

        Parameters
        ----------
        bundle_type : 'collection' (default, read-only) | 'transaction' (write)
        ...
        >>> print(json.dumps(bundle, indent=2)[:300])
        """
        from digitalsoma.fhir.fhir_io import to_fhir_bundle as _to_fhir_bundle
        return _to_fhir_bundle(self, bundle_type=bundle_type)
