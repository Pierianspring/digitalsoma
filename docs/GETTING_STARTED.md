...
```python
import digitalsoma
print(digitalsoma.__version__)   # → 2.1.0
```
...
DigitalSoma v2.1.0 adds a native HL7 FHIR R4 export. Every twin can produce a standards-compliant Bundle containing a Patient (animal subject), Device (twin identifier), Observations (one per mapped canonical property), and a DiagnosticReport (VeDDRA findings):
